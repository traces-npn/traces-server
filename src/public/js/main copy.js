...
function dibuixaTracksOnline() {
    ...
    // Consulta posicions a l'API de Traccar
      ...
      fetch(url,options)
          ...
          .then(json => {     
            ...
            let x=json[0].latitude;
            let y=json[0].longitude;
            let z=json[0].altitude;
            let t=json[0].deviceTime;          
  ...
  */                           
  ...
  function actualitzarPosicions_prova(id) {
    // Consulta posicions a l'API de Traccar
      const url = `http://ossera:8082/api/positions?deviceId=${id}`;    
      const options = {
          "method": "GET",
          "headers": {
          "Authorization": "Basic YWRtaW46YWRtaW4="
