...
const distMinProximitat = 50;  // Metres de tolerància de proximitat als controls.
var posicio='';
...
function getPosition(id,dorsal) {  
  ...
  const req = http.request(options, function (res) {            
    ...
    res.on("end", function () {
      ...
      const batteryLevel = jsonParsed[0].attributes.batteryLevel;
      const motion = jsonParsed[0].attributes.motion;
      const lat =  jsonParsed[0].latitude;
      const lng =  jsonParsed[0].longitude;
      const alt =  jsonParsed[0].altitude;
      const time = jsonParsed[0].fixTime;
      const t=timeStamp;                      
            ...
            if (controlsCorredors.length == 0)  {  
              ...
              getControlsCorredors(id, 1, function (controlCim) {   
                if (controlCim.length > 0) {  
                  // Si ja s'ha passat pel control de cim es registre l'arribada sinó es descarta
